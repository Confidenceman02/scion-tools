...
func getNodeStackHelp[K any, V any](targetKey Comparable[K], ns *nodeStack[Comparable[K], V]) Maybe[*node[Comparable[K], V]] {
	switch targetKey.Cmp(ns.node.key.T()) {
	case -1:
		if ns.node.left == nil {
			return Nothing{}
		} else {
...
// 	// Handle x's parent
// 	if stk.p != nil {
// 		pSide := parentSide(&nodeStack[basics.Comparable[K], V]{node: x, stack: stk})
//
// 		switch pSide {
// 		case LEFT:
// 			stk.p.left = left
